...
def solve4temp(state, hIceActual, hSnowActual, TSurfIn, TempFrz):
    ...
    def fluxes(t1):
        ...
        dFia_dTs = npx.where(isIce, 4 * d3 * t3 + d1 * ug
                                    + d1i * ug * dqs_dTs, 0)
        #TODO remove last three variables
        return F_c, F_lh, F_ia, dFia_dTs, F_lwu, F_sens, q_s

    # iterate for the temperatue to converge (Newton-Raphson method)
    for i in range(6):

        F_c, F_lh, F_ia, dFia_dTs, F_lwu, F_sens, q_s = fluxes(TSurfLoc)
